...
def ind2sub(shape, ind):
    assert (ind < shape[0]*shape[1])
    return ind//shape[1], ind%shape[1]
...
'''
def DrawClassyPic(x, y, classy_fun=None, x_label='x', y_label='y',title='标题'):
    if x.shape[0] == 2:
        x = x.T
...
def switch_container( data ):
    ...
    if isinstance(data, str):
        ...
        ret = dict((switch_container(k), switch_container(v)) for k, v in data.items())
    else:
        ret = data
    return ret
...
def plotNode(nodeTxt, centerPt, parentPt, nodeType):
    createPlot.ax1.annotate(str(nodeTxt), xy=parentPt, xycoords='axes fraction', \
